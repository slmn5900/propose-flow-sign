...
const EstimateForm = ({ estimate, onSuccess, onCancel }: EstimateFormProps) => {
  const { user } = useAuth();
  const [loading, setLoading] = useState(false);
  const [items, setItems] = useState<EstimateItem[]>(
    estimate?.items || [{ description: '', quantity: 1, rate: 0, amount: 0 }]
  );
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle>{estimate ? 'Edit Estimate' : 'Create New Estimate'}</CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
